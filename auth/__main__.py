...
from starlette.status import HTTP_201_CREATED, HTTP_400_BAD_REQUEST, HTTP_404_NOT_FOUND, HTTP_422_UNPROCESSABLE_ENTITY
...
async def register(db_session: DBAsyncSession, data: UserRegisterDTO):
    user = await db_session.scalar(
        select(User).filter(and_(User.email == data.email))
    )
    if user:
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST,
            detail=f"user with email={data.email} exist"
        )
